...
/**
 * <p>This filter can, based on HTTP headers in a {@link HttpServletRequest},
 * compress data written to the {@link
 * HttpServletResponse}, or decompress data read from the request. When
 ...
 * {@link Filter}, this class can also be easily added to any J2EE 1.3+ web
 * application.</p>
 *
 * <h3>Features</h3>
 *
 ...
 * <ul>
 *
 * <li><strong>debug</strong> (optional): if set to "true", additional debug
 * information will be written to the servlet log. Defaults to false.</li>
 *
 * <li><strong>compressionThreshold</strong> (optional): sets the size of the
 * smallest response that will be compressed, in bytes. That is, if less than
 * {@code compressionThreshold} bytes are written to the response, it will not
 ...
 *
 * <li><strong>includeContentTypes</strong> (optional): if specified, this is
 * treated as a comma-separated list of content types (e.g.
 * {@code text/html,text/xml}). The filter will attempt to only compress
 * responses which specify one of these values as its content type, for example
 * via {@link
 * HttpServletResponse#setContentType(String)}. Note that the filter does not
 ...
 * that this parameter cannot be specified if {@code excludeContentTypes} is
 * too.</li>
 *
 * <li><strong>excludeContentTypes</strong> (optional): same as above, but
 * specifies a list of content types to <strong>not</strong> compress.
 * Everything else will be compressed. However note that any content type that
 * indicates a compressed format (e.g. application/gzip, application/x-compress)
 * will not be compressed in any event.</li>
 *
 * <li><strong>includePathPatterns</strong> (optional): if specified, this is
 * treated as a comma-separated list of regular expressions (of the type
 * accepted by {@link Pattern}) which match exactly those paths which should be
 ...
 * using standard {@code filter-mapping} elements in {@code web.xml}; this
 * element provides more fine-grained control for when that mechanism is
 * insufficient. "Paths" here means values returned by
 * {@link HttpServletRequest#getRequestURI()}. Note that the regex must match
 * the filename exactly; pattern "static" does <strong>not</strong> match
 * everything containing the string "static. Use ".*static.*" for that, for
 * example. This cannot be specified if {@code excludeFileTypes} is too.</li>
 *
 * <li><strong>excludePathPatterns</strong> (optional): same as above, but
 * specifies a list of patterns which match paths that should
 * <strong>not</strong> be compressed. Everything else will be compressed.</li>
 *
 * <li><strong>includeUserAgentPatterns</strong> (optional): Like
 * {@code includePathPatterns}. Only requests with {@code User-Agent} headers
 ...
 * filter.</li>
 *
 * <li><strong>jakartaCommonsLogger</strong> (optional): if specified the named
 * Jakarta Commons Log will also receive log messages from this filter.</li>
 *
 * </ul>
 *
 ...
 * <h4>Response</h4>
 *
 * <p>This filter supports the following compression algorithms when compressing
 * data to the repsonse, as specified in the "Accept-Encoding" HTTP request
 * header:</p>
 *
 * <ul>
 * <li>gzip</li>
 ...
 * <h4>Request</h4>
 *
 * <p>This filter supports the following compression algorithms when
 * decompressing data from the request body, as specified in the
 * "Content-Encoding" HTTP request header:</p>
 *
 * <ul>
 * <li>gzip</li>
 * <li>x-gzip</li>
 ...
 *
 * <p>An application may force the encoding / compression used by setting an
 * "Accept-Encoding" value into the request as an attribute under the key
 * {@link #FORCE_ENCODING_KEY}. Obviously this has to be set upstream from the
 * filter, not downstream.</p>
 *
 * <h3>Caveats and Notes</h3>
 *
 * <p>The filter requires Java 5 and J2EE 1.4 or better.</p>
 *
 * <p>Note that if this filter decides that it should try to compress the
 * response, it <em>will</em> close the response (whether or not it ends up
 * compressing the response). No more can be written to the response after this
 * filter has been applied; this should never be necessary anyway. Put this
 * filter ahead of any filters that might try to write to the repsonse, since
 * presumably you want this content compressed too anyway.</p>
 *
 * <p>If a {@link java.io.OutputStream#flush()} occurs before the filter has
 * decided whether to compress or not, it will be forced into compression
 * mode.</p>
 *
 * <p>The filter will not compress if the response sets
 * {@code Cache-Control: no-transform} header in the response.</p>
 *
 * <p>The filter attempts to modify the {@code ETag} response header, if
 ...
 * return the response again.</p>
 *
 * <p>The filter always sets the {@code Vary} response header to indicate that a
 * different response may be returned based on the {@code Accept-Encoding}
...
public final class CompressingFilter implements Filter {
     ...
     * ServletRequest} passed to this filter, under this key. The value should
     * be a valid "Accept-Encoding" header value, like "gzip". Specify
     * "identity" to force no compression.
     *
     * @since 1.2
     */
     ...
     * filter has applied compression to the response. Upstream filters may
     * check for this flag. Note that if the response has been compressed, then
     * it will be closed by the time this filter finishes as well.
     *
     * @since 1.2
     */
    ...
    /**
     * <p>Adds the "Vary" header; must be applied if the response <b>could be
     * compressed</b>, not <b>if it is compressed</b>.</p>
     *
     * <p>This is necessary to make sure the response is not cached (e.g. by Web
    ...
    /**
     * Checks to see if the given path should be compressed. This checks against
     * the {@code includePathPatterns} and {@code excludePathPatterns} filter
     * init parameters; if the former is set and the given path matches a
     * regular expression in that parameter's list, or if the latter is set and
     * the path does not match, then this method returns {@code true}.
     *
